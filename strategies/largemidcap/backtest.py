...
headers = ["Metric", "Strategy", "Benchmark", "Edge"]
row_h   = 0.082
y_start = 0.92

for xi, h in zip(col_x, headers):
    ax.text(xi, y_start, h, transform=ax.transAxes, fontsize=10,
            fontweight="bold", color=WHITE)
# thin divider below header row
ax.plot([0, 1], [y_start - row_h * 0.15, y_start - row_h * 0.15],
        transform=ax.transAxes, color=GRID, lw=1.2)
...
fig.text(0.02, 0.01,
         "Benchmark: Equal-Weight Nifty LargeMidcap 250 — all 250 index constituents held in equal weight, rebalanced monthly.",
