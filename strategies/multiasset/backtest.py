...
# ─────────────────────────────────────────────────────────────────────────────
# CHART 1 — Performance scorecard (rebalanced typography)
# ─────────────────────────────────────────────────────────────────────────────
# Public 5Y reference metrics for the benchmark — kept centrally in
# shared.config.BENCHMARKS so they can be refreshed in one place.
B_VOL   = BENCH_CFG["vol"]
B_MDD   = BENCH_CFG["max_dd"]
B_BEST  = BENCH_CFG["best_month"]
B_TOTAL = (1 + BENCH_CAGR) ** ((price_df.index[-1] - price_df.index[MIN_START]).days / 365.25) - 1
...
# Hero row — three KPIs (smaller, more proportionate)
hero = [
    ("STRATEGY CAGR",   f"{mt['CAGR']*100:.1f}%",        ACCENT2),
    ("BENCHMARK CAGR",  f"{BENCH_CAGR*100:.2f}%",        GRAY),
    ("ALPHA (annual)",  f"{(mt['CAGR']-BENCH_CAGR)*100:+.2f}%",
                        GREEN if mt['CAGR'] > BENCH_CAGR else RED),
]
hero_y = 0.83
...
# Metrics grid — every benchmark cell now populated from public 5Y data.
metrics_rows = [
    ("Total return",  f"{mt['Total Return']:.1%}",       f"{B_TOTAL:.1%}",            True),
    ("Volatility",    f"{mt['Volatility']:.1%}",         f"{B_VOL:.1%}",              False),
    ("Sharpe ratio",  f"{mt['Sharpe Ratio']:.2f}",       f"{B_SHARPE:.2f}",           True),
    ("Max drawdown",  f"{mt['Max Drawdown']:.1%}",       f"{B_MDD:.1%}",              False),
    ("Calmar ratio",  f"{mt['Calmar Ratio']:.2f}",       f"{B_CALMAR:.2f}",           True),
    ("Win rate",      f"{mt['Win Rate']:.1%}",           f"55.0%",                    True),
    ("Best week",     f"{mt['Best Week']:.1%}",          f"{B_BEST:.1%}",             True),
    ("Final value",   f"₹{mt['Final Value']/100000:.1f}L",
                      f"₹{(INITIAL_AUM/100000) * (1+B_TOTAL):.1f}L",                  True),
]

# Table — slightly larger rows, larger fonts for legibility
table_top = 0.68
row_h     = 0.075
