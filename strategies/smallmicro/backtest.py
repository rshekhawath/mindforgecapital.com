...
from shared.config import SMALLMICRO, BENCHMARKS, RISK_FREE
...
# The Nifty Microcap 250 launched Jan 2024, so a true 5Y series for the
# combined Smallcap 250 + Microcap 250 index is not available. We therefore
# use the historical 5Y CAGR from public NSE / Screener reference data
# (configured centrally), and render the benchmark as a smooth exponential
# at this CAGR. Strategy logic is unchanged.
BENCH_CFG    = BENCHMARKS["smallmicro"]
...
for i, d in enumerate(rebal_dates[:-1]):
    ...
    # Benchmark return — static per-period rate derived from BENCH_CAGR.
    period_years = (next_d - d).days / 365.25
    br = (1.0 + BENCH_CAGR) ** period_years - 1.0
...
p_cagr   = stats["p_cagr"];   b_cagr   = BENCH_CAGR  # static, from config
...
STRAT_NAME    = "MultiFactor SmallMicro 500"
...
# ─────────────────────────────────────────────────────────────────────────────
# CHART 3 — Rolling 12-Month Performance (CAGR + Sharpe)
# ─────────────────────────────────────────────────────────────────────────────
window = 12
...
ax = axes[0]
ax.plot(rc_p.index, [BENCH_CAGR * 100] * len(rc_p), color=BNH_COLOR,
        lw=1.6, ls=(0, (4, 3)),
        label=f"{BENCH_SHORT}  ·  {BENCH_CAGR*100:.2f}% CAGR (static)")
ax.plot(rc_p.index, rc_p, color=STRATEGY_COLOR, lw=2.2, label=STRAT_NAME)
...
ax = axes[1]
ax.plot(rs.index, rs, color=STRATEGY_COLOR, lw=2.2, label=STRAT_NAME)
ax.axhline(0, color=GRAY, lw=0.6)
