#!/bin/bash
# ════════════════════════════════════════════════════════════════
#  MindForge Capital — Run SmallMicro 500 Strategy
# ════════════════════════════════════════════════════════════════
#  WHEN TO RUN:
#    • A subscriber just paid for "MultiFactor SmallMicro 500"
#    • You want to refresh portfolio picks before activating them
#    • Monthly rebalance day (run this, then activate all subscribers)
#
#  WHAT IT DOES:
#    1. Scores all 500 stocks in the SmallMicro universe
#    2. Selects top 50 (max 3 per sector)
...
UNIVERSE_PATH="$ROOT_DIR/strategies/smallmicro/universe.xlsx"

GREEN='\033[0;32m'; YELLOW='\033[1;33m'; RED='\033[0;31m'; BOLD='\033[1m'; NC='\033[0m'

echo ""
echo -e "${BOLD}╔══════════════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}║   MindForge Capital — SmallMicro 500 Strategy Runner     ║${NC}"
echo -e "${BOLD}╚══════════════════════════════════════════════════════════╝${NC}"
echo ""
...
if [ ! -f "$UNIVERSE_PATH" ]; then
    ...
    echo "  $UNIVERSE_PATH"
    echo ""
    echo "  Expected at: strategies/smallmicro/universe.xlsx"
    echo ""
...
# ── Run the strategy ────────────────────────────────────────────
echo -e "${BOLD}Running SmallMicro 500 factor model…${NC}"
echo "  (This takes 5–10 minutes — downloading 500 tickers from Yahoo Finance)"
echo ""
...
try:
    from mindforge_runner import run_smallmicro
    stocks = run_smallmicro()
    ...
    if stocks:
        ...
        print(f"  Strategy: SmallMicro 500")
        print("━" * 60)
        print("")
        print("  NEXT STEP:")
