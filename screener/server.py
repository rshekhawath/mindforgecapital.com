...
# ──────────────────────────────────────────────────────────────────────────────
# Full NSE EQ-series universe (2120 stocks)
# Yahoo Finance supports SYMBOL.NS for every entry.
# The screener fetches data on-demand and caches for 1 hour.
# V37.9: NSE renames a listing without notice and Yahoo keeps serving the OLD
# symbol at a frozen price for ~2 months before it 404s. Renamed in place, per
# NSE symbolchange.csv: GUJGASLTD->GUJENERGY, LYPSAGEMS->AURUS,
# AMIRCHAND->AEROPLANE, ASHIKA->ASHIKAG, SILLYMONKS->CRESTO.
# refresh_data.py checks that file on every run and names any new rename.
# V37.9a: removed AURIGROW, JBCHEPHARM, WIMPLAST (off NSE's EQUITY_L.csv with
# no symbol change; Yahoo price frozen since Jul/Jun 2026).
# ──────────────────────────────────────────────────────────────────────────────
NSE_EQUITY_UNIVERSE = [
    ...
    "AMANTA","AMBALALSA","AMBER","AMBICAAGAR","AMBIKCO","AMBUJACEM","AMDIND","AEROPLANE","AMJLAND","AMNPLST",
    ...
    "ASAL","ASALCBR","ASHAPURMIN","ASHIANA","ASHIKAG","ASHIMASYN","ASHOKA","ASHOKAMET","ASHOKLEY","ASIANENE",
    ...
    "ATULAUTO","AUBANK","AURIONPRO","AUROPHARMA","AURUM","AUTOAXLES","AVALON","AVANTEL","AVANTIFEED",
    ...
    "GUJALKALI","GUJAPOLLO","GUJENERGY","GUJRAFFIA","GUJTHEM","GULFOILLUB","GULFPETRO","GULPOLY","GVPIL","GVPTECH",
    ...
    "JBMA","JETFREIGHT","JGCHEM","JHS","JINDALPHOT","JINDALPOLY","JINDALSAW","JINDALSTEL","JINDRILL",
    ...
    "LUPIN","LUXIND","LXCHEM","LYKALABS","AURUS","M&M","M&MFIN","MAANALU","MACPOWER","MADHAV",
    ...
    "SIGNPOST","SIKA","SIKKO","SIL","SILGO","SILINV","CRESTO","SILVERTUC","SIMPLEXINF","SINCLAIR",
    ...
    "WENDT","WESTLIFE","WEWORK","WHEELS","WHIRLPOOL","WILLAMAGOR","WINDLAS","WINDMACHIN","WIPL",
...
def fetch_stock_info(symbol):
    ...
    try:
        ...
        # yfinance does not raise on a Yahoo 404 (renamed/delisted symbol); it
        # returns a near-empty .info, which used to be cached and published as a
        # blank row counted "ok". Report it as a failure and keep the old row.
        if not (i.get("currentPrice") or i.get("regularMarketPrice")) \
                and not (i.get("longName") or i.get("shortName")):
            conn.close()
            return {"symbol": symbol,
                    "error": "Yahoo has no quote for this symbol (renamed or delisted on NSE?)"}
