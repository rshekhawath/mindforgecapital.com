...
def style_ax(ax, title="", xlabel="", ylabel=""):
    ...
    if title:
        ax.set_title(title, color=WHITE, fontsize=13, fontweight="bold",
                     pad=12, loc="left")
    if xlabel:
        ax.set_xlabel(xlabel, color=LGRAY, fontsize=11)
    if ylabel:
        ax.set_ylabel(ylabel, color=LGRAY, fontsize=11)
...
def fmt_xaxis(ax, interval=6):
    ...
    plt.setp(ax.xaxis.get_majorticklabels(), rotation=30, ha="right",
             color=LGRAY, fontsize=11)


def legend(ax, **kwargs):
    """Attach a styled legend to the Axes."""
    leg = ax.legend(fontsize=10, framealpha=0.7, labelcolor=WHITE,
